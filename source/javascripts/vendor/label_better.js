...
!function($){
  ...
  $.fn.label_better = function(options){
    ...
    el.each(function( index, value ) {
      ...
      btn.wrapAll("<div class='lb_wrap' style='position:relative;padding-bottom: 1.5em;'></div>")

      if( btn.val().length > 0) {
        var text = btn.data("new-placeholder")  || btn.attr("placeholder");
        $("<small class='lb_label u-font-sans " + position + "'>"+ text + "</small>").css("opacity", "0").insertAfter(btn).animateLabel(settings, btn);
      }

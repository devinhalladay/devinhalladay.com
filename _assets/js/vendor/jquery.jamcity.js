  ...
  */
  ...
  JamCity_Construct.prototype = {
    ...
    templateData: function (options, json_item) {
      ...
      switch (options.contentType) {
        case 'recentTracks':
          ...
          o.item_name = '<em>' + json_item.name + '</em> by ' + json_item.artist['#text'];
    ...
    buildHTML: function (items, el, options) {
      var jamcity = this,
        item_image = null;
      $(el).append('<p class="jams__jam"></p>');
      if (items == 'jamCityError') {
        $(el).find('.jams__jam').addClass('jamCityError').append('<p>' + options.noResults + '</p>');
      } else {
